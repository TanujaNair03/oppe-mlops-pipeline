...
- Creates a binary target if not present: target = (next_close > close)
...
def main():
    ...
    # Ensure target column exists. Use ts_col variable defined earlier to sort
    if target_col not in df.columns:
        print(f"[INFO] '{target_col}' not found. Creating target as (next_close > close).")
        df = df.sort_values(by=[ts_col])
        # create next_close within symbol groups to avoid cross-symbol leakage
